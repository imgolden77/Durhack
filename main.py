...
# -----------------------------
# Simple car data (demo)
# Replace with your real database if you have one
# -----------------------------
# comb08 = EPA combined MPG (miles per gallon)
CAR_DATA: Dict[str, Dict[str, Dict[str, int]]] = {
    # Make -> Model -> MPG
    "Toyota": {
        "Corolla": 32,
        "Camry": 28,
...
@app.get("/makes")
def makes(year: Optional[int] = None):
    # For demo we ignore year and return all makes
    return {"makes": sorted(CAR_DATA.keys())}


@app.get("/models")
def models(year: Optional[int] = None, make: str = Query(...)):
    if make not in CAR_DATA:
        return {"models": []}
    return {"models": sorted(CAR_DATA[make].keys())}


@app.get("/results")
def results(year: Optional[int] = None, make: str = Query(...), model: str = Query(...)):
    if make not in CAR_DATA or model not in CAR_DATA[make]:
        return {"results": []}
    mpg = CAR_DATA[make][model]
    # mirror your front-end expectation: results list with an object that has comb08
    return {
        "results": [
            {
                "make": make,
